...
const InteractivePlayerTable = () => {
    ...
    const [dragOverInfo, setDragOverInfo] = useState({ index: null, tier: null });
    ...
    const handleDragOver = (e, index, tier) => {
        e.preventDefault();
        if (draggedItem === null) return;
        setDragOverInfo({ index, tier });
    };

    const handleDragLeave = (e) => {
        if (!e.currentTarget.contains(e.relatedTarget)) {
            setDragOverInfo({ index: null, tier: null });
        }
    };
    
    const handleDrop = (e) => {
        e.preventDefault();
        if (!draggedItem || dragOverInfo.index === null) {
            handleDragEnd();
            return;
        }
